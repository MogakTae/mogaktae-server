package code.mogaktae.challenge.controller.api;

import code.mogaktae.auth.domain.UserDetailsImpl;
import code.mogaktae.challenge.controller.docs.ChallengeControllerSpecification;
import code.mogaktae.challenge.dto.req.ChallengeCreateRequest;
import code.mogaktae.challenge.dto.req.ChallengeJoinRequest;
import code.mogaktae.challenge.dto.req.ChallengeSummariesRequest;
...
import code.mogaktae.challenge.facade.ChallengeFacade;
...
public class ChallengeController implements ChallengeControllerSpecification {
    ...
    @PostMapping("/commit")
    public ResponseEntity<ResponseDto<?>> handleProblemSolveCommit(@RequestBody Map<String, Object> request){
        return ResponseEntity.status(HttpStatus.OK).body(ResponseDto.of(challengeFacade.handleProblemSolveCommit(request), "사용자 문제 풀이 처리 완료"));
    }
}
