...
public class AlarmController implements AlarmControllerSpecification {
    ...
    @GetMapping
    public ResponseEntity<ResponseDto<AlarmResponseDto>> getAllAlarm(@AuthenticationPrincipal OAuth2UserDetailsImpl user){
        return ResponseEntity.status(HttpStatus.OK).body(ResponseDto.of(alarmService.getAlarms(user.getName()), "알림 조회 성공"));
    }
}
