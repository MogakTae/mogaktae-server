...
public class BaekjoonClient {
    ...
    public String getSolvedProblemIds(String solvedAcId){

        String url = "https://www.acmicpc.net/user/" + solvedAcId;
        StringBuilder solvedProblemIds = new StringBuilder();

        try{
            Document document = Jsoup.connect(url).get();

            Elements elements = document.select("div.problem-list a");

            for(Element link : elements){
                String problemId = link.text().trim();

                if (!problemId.isEmpty())
                    solvedProblemIds.append(problemId).append(" ");
            }
        }catch(IOException e){
            throw new RestApiException(CustomErrorCode.PARSE_CRAWLING_DATA_FAILED);
        }

        return solvedProblemIds.toString();
    }
